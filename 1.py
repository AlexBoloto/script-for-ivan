...
def maintain_df(data_frame,zhk):
    ...
    data_frame['Rooms'].replace({'0': 'СТ', '1': '1K', '2': '2К', '3': '3К', '4': '4К'}, inplace=True)
    print(data_frame['Rooms'])
...
def merge(data_frame, file):
    ...
    for i in range(len(mer_df)):
        ...
        #else: mer_df.iloc[i, mer_df.columns.get_loc('Отделка_y')]=0
    ...
    mer_df['Rooms'].replace({0: 'CT', 1: '1K', 2: '2K', 3: '3K', 4: '4K'}, inplace=True)
